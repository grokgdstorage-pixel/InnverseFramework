...
kit: Innverse
path: frameworks/magic/spellcraft.md
parent: frameworks/magic/how-magic-works.md
status: draft
---
...
Mana-path **teaching**. How a [Mage]-line caster learns a spell, rebuilds one, or invents a name.

Read `how-magic-works.md` first. Source, manner, boxed vs freeform, spec-drift, and quality live there. This file does not rewrite them.

If a Class name might change, `classes.md`. If a level might fire, `leveling.md`.
...
- what magic is
- a pattern sitting in an object, a presence arriving, who it obeys
- witch / shaman / druid manuals
- a spell list or Class list
- Wistram politics

## How to use

Open this file when they are **learning**, **rebuilding**, or **inventing** a spell.

If they already have the template and are only throwing it (`[Fireball]` they already know), stay in the parent. Do not open this file to re-teach the box.

If they are putting that spell into an object, leave after naming the effect. Object-founding is not this file.
...
7. Whether the parent, `classes.md`, or `leveling.md` should open

Default: options plus reasons. Oversupply is fine. Narrator cuts. Do not lock List A to List B.
...
- No spell directory. No Class directory.
- No gold-per-tier table.
- Do not invent a prodigy from a silent profile.
- Do not re-teach boxed vs freeform.
- Do not found a golem, scroll, or corpse here.
...
From that pool the offer may use:

- **Deed** — what they actually did with magic
- **Need** — what the moment required
- **Desire** — what they were reaching for
- **Mind** — how they understand the spell (Calculated / Felt is the parent)
...
|---|---|---|
| Foundation | Theory, and a pool that can hold the next step | Parent. Pointer below. |
| Acquisition | Learn a template that already exists | This file. |
| Materialization (throw) | Fill a box they already have | Short section here. Window and grade: parent. |
| Materialization (object) | Put the pattern into a thing | Not this file. |
| Theorization | Rebuild or invent | This file. |
...
They need a pool and enough theory to take the next step. How large the pool is, how they think (Calculated / Felt), and whether they are average or gifted: parent + profile.
...
Learning a hard spell can itself be a deed `leveling.md` weighs. One new spell is not automatically a level.

---

## Fill the box

They already have the template. They throw it now.

This section exists so a known `[Fireball]` is not treated as Theorization. Crude vs practiced grade of the *same* box lives in the parent. Do not rewrite it. Do not invent a new name for a throw they can already do.

If the question is only that throw, the parent was enough. If they then put that same effect into an object, stop. That is not this file.

---

## Theorization

Build or rebuild the spell. Rare.
...
- List A first (what it does)
- List B second (what it might be called)
- Narrator pairs them
...
## When this can feed a Class step

One learned or invented spell is not a level by itself.

Living this — nights on a hard template, a rebuild that costs them, a failed invention they then do honestly — can be a deed. `leveling.md` owns height. `classes.md` owns the name.
...
- Exact book-copy / scribe height (point at `classes.md` or a later note)
- Witch / shaman / druid making-magic, different source
